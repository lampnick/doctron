...
func (ins *html2pdf) Convert() ([]byte, error) {
	...
	firstPageWait := 2 * time.Second
	lastPageWait := 2 * time.Second

	err := chromedp.Run(ctx, chromedp.Tasks{
		chromedp.Navigate(ins.cc.Url),
		chromedp.Sleep(firstPageWait),
	})
	if err != nil {
		return nil, err
	}
	...
	totalWaitTime := time.Duration(params.WaitingTime)*time.Millisecond - firstPageWait - lastPageWait
	waitEachPage := 500 * time.Millisecond
	for i := time.Duration(0); i < totalWaitTime; i += waitEachPage {
		// Simulate pressing the Page Down key
		err := chromedp.Run(ctx, chromedp.Tasks{chromedp.KeyEvent(kb.PageDown)})
		if err != nil {
			return nil, err
		}

		// Wait for a while to ensure that the page has finished loading
		err = chromedp.Run(ctx, chromedp.Sleep(waitEachPage))
		if err != nil {
			return nil, err
		}
		...
		err = chromedp.Run(ctx,
			chromedp.EvaluateAsDevTools(
				// From https://techstacker.com/javascript-detect-when-scrolled-to-bottom/
		...
		// Exit the loop if the page has scrolled to the bottom
		if isBottom {
			break
		}
	}

	err = chromedp.Run(ctx, chromedp.Sleep(lastPageWait))
	if err != nil {
		return nil, err
	}
